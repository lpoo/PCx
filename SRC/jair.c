...
void jair1(MMTtype *M, FactorType *Factor, double b[], double c[], double *px, double *pi, double *ps, double *pw, double *pr, double upbound[], int NumBounds, int BoundIndex[], double **xini)
{
    ...
    double *x, *s, *w, *r;
    double vv1, c1, c11, s1, somat,  v, alpu, alpl1, auxl1, ep1, ep2, resiff, resif1, dif, rr3, normabc,d__1,absxi,scale, aux55,nhp, param_tt ;
...
//    param_tt=param_t;
    ...
    //printf(" pp2 p aux5 aux55 %d %d %d %f \n",pp2,p, aux5,aux55);
...
//até aqui
    ...
    for(i = 0; i < NumBounds; i++)
    {
        ...
        //r[i]      *= normau[i];     //mudei aqui
    }

    normabc  = sqrt(c1);    // norma de bc

    //vv1     *= normabc;        // fazendo transformação na última variavel v // mudei aqui
    //somat    = vv1; //mudei aqui
...
//até aqui
    ...
    *resiv = 0;
    ...
    for(i = 0; i < NumBounds; i++)
    {
        //w[i]                     /= somat; //mudei aqui
        //r[i]                     /= somat; //mudei aqui
    ...
    while(zz == 1)
    {
        ...
        //if((dif<0.0001)||(cc>ccc))
        if(cc>ccc)
        {
            //end = clock();

            //printf( "tempo: %50.50g seconds\n",(double)(end - start) / CLOCKS_PER_SEC);
//            printf("resifffim %11.10lf %d %d \n ", resiff,p, cc-1);
...
*/
        ...
        if((cc % 2) == 0)
        {
              ...
              else
            {
                ...
                //r1r2[aux5]=wwa[aux1-xzero-aux5-1]; //3-10-2013

                for(i = 0; i < aux5; i++)
                {
                    r1r2[i]      = wwa[aux1 - xzero - i - 1];    // ind dos (p/2) vetores que formam o maior com residuo
                    //r1r2[1+i+aux5] = wwa[i];        //3-10-2013        // indices dos (p/2) vetores que formam o menor ang com resi
                }
/*
                         for (i=0; i<p; i++)
                          {
        ...
        else
        {
            ...
            {
                //r1r2[aux5] = wwa[aux1 - xzero - aux5-1]+aux1; //3-10-2013

                for(i = 0; i < aux5; i++)
                {
                    r1r2[i]      = wwa[aux1 - xzero - i-1]+aux1;    // ind dos (p/2) vetores que formam o maior com residuo
                    //r1r2[1+i+aux5] = wwa[i]+aux1;    //3-10-2013                    // indices dos (p/2) vetores que formam o menor ang com resi
                }
        ...
        if(zz == 1)
        {
            ...
            for(i = 0; i < aux5; i++)
            {
                ...
                //xgrande[r1r2[i+aux5]] = l1[aux5 + i + 1]; //3-10-2013
            }
            //xgrande[r1r2[p-1]]      = l1[p]; //3-10-2013
        ...
        if(cc == 1)
        {
            *xini = (double *)malloc(sizeof(double) * M->NumCols);
            for(i = 0; i < M->NumCols; i++)
            {
                (*xini)[i] = xgrande[i];
            }
...
void jair2(MMTtype *M, FactorType *Factor, double b[], double c[], double *px, double *pi, double *ps, double *pw, double *pr, double upbound[], int NumBounds, int BoundIndex[], double **xini)
{
    int    i, aux, auxx, aux1, aux2, aux3, aux4, aux5, aux6, aux7,ccc;
