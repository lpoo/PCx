...
extern int TROCA, LLt, Eta, trocou, marca;
extern FILE *eigout;
     ...
     Parameters     *Inputs;
{
   ...
   FactorType     *Factor1, *SplitFactorType();

   double         *b, *xib, *xis, *xibound, *scale, *scaleSparse,
                   ComputeCentering();
   double         *xini1, *xini2, *xdif, *xinidif;
   ...
   Iterate        *NewIterate(), *Current = NULL,	/* Current Solution */
                  ...
                  *Copyvon = NULL,      // copia para Von Neumann
  ...
  /* Copy the matrix and rhs into more congenial data structures */
   ...
   Copyvon = NewIterate(NumRows, NumCols, NumBounds);
   ...
   xdif = (double *)malloc(sizeof(double) * NumCols);
   xinidif = (double *)malloc(sizeof(double) * NumCols);
  ...
  /*******************************************************************/
   ...
   else
   {
     ...
     Asparse = Adense = 0;
     scaleSparse = 0;
     Factor = SplitFactorType(Asparse, A->NumRows, A->NumCols);
     /* Na funcao create-LU e' calculada estrutura de ADAt e alocadada memoria para L */
     create_LU(A,Factor);
   }
   strcpy(Solution->FactorizationCode, Factor->FactorizationCode);
	...
	* variables, if necessary, to keep the two of them from blowing up */
...
#ifdef CGM
       if(!LLt)
         parametros(Iteration,mu,Factor);
	 ...
	 else
	 {
       ...
       printf("PriInf: %7.5e \n",PriInf);
       FacPrim= fabs(primal_objective - dual_objective) /  (1.0 + (fabs(primal_objective)));
       printf("Variacao do mu %7.5e,\n", rmu_ratio / rmu_ratio0);
       if (FacPrim <= 9e-5) marca=1;
       printf("Criterio  PriFeasTol: %7.5e \n", PriFeasTol);

       /* Verifica factibilidade do primal e do Dual e a diferença entre ambos valores*/
       /* Declare OPTIMAL status*/
	 ...
	 if((PriInf >= PriFeasTol || DualInf >= DualFeasTol) &&
	   ...
	   printf(" (due to large residual/mu ratio)\n");
           fprintf(eigout, "\n--termination with UNKNOWN status");
           fprintf(eigout, " (due to large residual/mu ratio)\n");
           printf("(current rmu)  = %e\n",rmu_ratio);
           printf(" (initial rmu) = %e\n",rmu_ratio0);
	 ...
	 printf(" (due to slow merit improvement)\n");
         fprintf(eigout,"\n--termination with UNKNOWN status ");
         fprintf(eigout," (due to slow merit improvement)\n");
	 algorithm_loop_flag = OFF;
	 break;
       }
	 ...
	 algorithm_loop_flag = OFF;
       ...
       /* parte referente ao precondicionador separador */
	 ...
	 int UM = 1;
         ...
         puts("Trocando de Precondicionador ...\n ");
         TROCA= 0;
         LLt= 0;

        // leitura do novo ponto

	ShiftSplitVariables(LP, Current);
        ComputeObjInf(A, b, LP->c, upbound, Current, xibound, xib, xis, &PriInf, &DualInf, &mu, &primal_objective, &dual_objective, &phi, &rmu_ratio);

        if(!LLt)
		parametros(Iteration,mu,Factor);
        mu     = mu / (NumCols + NumBounds);

        digits = floor(-log10(fabs(primal_objective - dual_objective) / (1.0 + fabs(primal_objective))));
	...
	else
	{
         ...
         if ( Ndense > 0 )
         {
           Asparse = NewMMTtype(A->NumRows, A->NumCols - Ndense, A->Nonzeros - nonzDense);
           StripA(A, Asparse, maskDense, 0);
           scaleSparse = NewDouble(NumCols-Ndense, "scaleSparse");
           Adense = NewMMTtype(A->NumRows, Ndense, nonzDense);
           StripA(A, Adense, maskDense, 1);
           Factor1 = NewFactorType(Asparse, Ndense, A->NumCols);
           A->ValueT = NewDouble(A->Nonzeros, "A->ValueT");
           status = TransposeSparseRealMatrix (A->Value, A->pBeginRow, A->pEndRow, A->Row,
                          A->ValueT, A->pBeginRowT, A->pEndRowT, A->RowT,
                          &(A->NumRows), &(A->NumCols));
           if (status)
             printf("Error: TransposeSparseRealMatrix = %d\n", status);
         ...
         else
         {
           Asparse = Adense = 0;
           scaleSparse = 0;
           Factor1 = NewFactorType(A, Ndense, A->NumCols);
         }
         Factor1->Ndense = Ndense;
         Free((char*) Factor1->maskDense);
         Factor1->maskDense = maskDense;

         status = Order(Factor1);
         if (status == FACTORIZE_ERROR)
         {
           printf("Error return from Order() routine.\n");
           printf("Aborting.\n");
           return status;
	 ...
	 if ( Factor->Ndense > 0 )
	 {
       ...
       /*precondicionador separador */
         ...
         splitting(scale,A,Factor,NULL,NULL,NULL,NULL,Iteration);
    ...
    /*******************************************************************/
       ...
       /* xibound e xib A */
#ifdef CGM
       ComputeCorrector(A, Factor, scale, Current, xis, xibound, xib, upbound,
#else
	 ...
	 CorrectorTime += UserTime2 + SysTime2 - OldUserTime2 - OldSysTime2;
       ...
       /*******************************************************************/
       ComputeStepFactor(Current, Corrector, &alpha_P, &alpha_D, Inputs);
       ...
       for (i = 0; i < NumCols; i++)
       {
         Current->x[i] -= alpha_P * Corrector->x[i];
         Current->s[i] -= alpha_D * Corrector->s[i];
       }
	
       for (i = 0; i < NumRows; i++)
	    Current->pi[i] -= alpha_D * Corrector->pi[i];
	
       for (i = 0; i < NumBounds; i++)
       {
	 Current->w[i] -= alpha_P * Corrector->w[i];
	 Current->r[i] -= alpha_D * Corrector->r[i];
       }
...
******************************************************************************/
   ...
   /*referente ao pre separador */
   else
     splitting(scale,A,Factor,NULL,NULL,NULL,NULL,0);
   /*termina aqui */
   ...
   *FactorTime = EndUserTime - BeginUserTime;
   ...
   if(LLt)
     ...
     /*pre separador*/
     solve(Factor, tempR1, pi, scale, 0,0);
     /*termina aqui */
   ...
   *SolveTime = EndUserTime - BeginUserTime;
   ...
   if(LLt)
     ...
     /*pre separador*/
     solve(Factor, tempR2, tempR1, scale, 0,1);
     /*termina*/
