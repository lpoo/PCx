...
#define ROW_INCREMENT  5
#define COL_INCREMENT  5
#define ENT_INCREMENT 10
#define FACTOR        0.1
  ...
  Parameters     *Inputs;
{
  ...
  if (GetLine(line, corefile, 200) == HEADERLINE) {
    ...
    printf("Missing NAME line in MPS file.\n");
    return BAD_INPUT;
  }
  ...
  while (GetLine(line, corefile, 200) == DATALINE) {
  ...
  while (GetLine(line, corefile, 200) == DATALINE) {
    ...
    if (strncmp(OldColumnName, name1, 10) != 0) {
      /* found new column */
...
rowname[j+1]= '\0';
  ...
  if (!NoObjectiveRowName(MPS) && (strcmp(&rowname[i], MPS->ObjectiveName) == 0)) {
  ...
  RowNumber = GetRowNumber(MPS, &rowname[i]);
  if (RowNumber == -1) {
  ...
  int             length;
{
  ...
  do {
    ...
    for (i = 0; i < length; i++)
      line[i] = ' ';
    fgets(line, length, file);	/* get line from file */

  } while (line[0] == '*');	/* discard comment lines */
...
for(jj= 4; line[jj] == ' ' ; jj++) ;
  string_copy(name1, &line[jj], 10);	/* characters  5 - 12 to name1 */
for( ; line[jj] != ' ' ; jj++) ;
for( ; line[jj] == ' ' ; jj++) ;
  string_copy(name2, &line[jj], 10);	/* characters 15 - 22 to name2 */
for( ; line[jj] != ' ' ; jj++) ;
for( ; line[jj] == ' ' ; jj++) ;
  string_copy(valstr, &line[jj], 12);
...
for( ; line[jj] != ' ' ; jj++) ;
for( ; line[jj] == ' ' ; jj++) ;
  string_copy(name3, &line[jj], 10);	/* characters 40 - 47 to name3 */
for( ; line[jj] != ' ' ; jj++) ;
for( ; line[jj] == ' ' ; jj++) ;
  string_copy(valstr, &line[jj], 12);
